...
import VendorSearchBar from '@/couple/components/VendorSearchBar'
import VendorPreviewModal from '@/couple/components/VendorPreviewModal'
import SearchFiltersPanel from '@/couple/components/search/SearchFiltersPanel'
import VendorsGridContent from '@/couple/components/search/VendorsGridContent'
import ResultsPagination from '@/couple/components/search/ResultsPagination'
...
export default function EnhancedSearchResults() {
  ...
  return (
    <CouplePageShell title="Find Your Perfect Vendors" subtitle="Browse verified vendors for your special day" bare={false}>
      <Box>
        {/* Search Bar */}
        <VendorSearchBar
      ...
      {selectedVendor && (
        <VendorPreviewModal
          ...
          vendor={{
            ...
            image: selectedVendor.images?.[0] || selectedVendor.image || '',
            rating: selectedVendor.rating,
            reviewCount: selectedVendor.reviewCount,
            price: String(selectedVendor.price),
            location: selectedVendor.location,
          }}
          isInShortlist={isShortlisted(selectedVendor.id)}
          onAddToShortlist={() => toggleShortlist(selectedVendor)}
          onRemoveFromShortlist={() => toggleShortlist(selectedVendor)}
          onPrevVendor={() => {
            const idx = filteredVendors.findIndex((v) => v.id === selectedVendor.id)
            if (idx > 0) setSelectedVendor(filteredVendors[idx - 1])
          }}
          onNextVendor={() => {
            const idx = filteredVendors.findIndex((v) => v.id === selectedVendor.id)
            if (idx >= 0 && idx < filteredVendors.length - 1) setSelectedVendor(filteredVendors[idx + 1])
          }}
          canGoPrev={filteredVendors.findIndex((v) => v.id === selectedVendor.id) > 0}
          canGoNext={filteredVendors.findIndex((v) => v.id === selectedVendor.id) < filteredVendors.length - 1}
        />
      )}
    </CouplePageShell>
  );
