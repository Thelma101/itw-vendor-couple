...
function mapApiTask(item: any): Task {
  return {
    id: item.id,
    title: item.title,
    notes: item.notes || '',
...
export default function Checklist() {
  ...
  const { form, updateForm, dialogOpen, closeDialog, openDialog, handleSave: saveForm } = useFormDialog(
    ...
    async (formData) => {
      if (apiReady) {
        try {
          const created = await planningApi.createChecklistItem({
            title: formData.title.trim(),
            notes: formData.notes.trim() || undefined,
            dueDate: formData.dueDate || undefined,
            completed: false,
          })
          save([...tasks, mapApiTask(created)])
          return
        } catch {
          /* local */
        }
  ...
  useEffect(() => {
    void (async () => {
      try {
        const rows = await planningApi.listChecklist()
        if (Array.isArray(rows) && rows.length) {
          save(rows.map(mapApiTask))
          setApiReady(true)
        }
      } catch (error) {
  ...
  const toggleTask = async (id: string) => {
    ...
    if (apiReady) {
      try {
        await planningApi.updateChecklistItem(id, { completed })
      } catch {
        /* local */
      }
    }
  ...
  const removeTask = async (id: string) => {
    if (apiReady) {
      try {
        await planningApi.deleteChecklistItem(id)
      } catch {
        /* local */
      }
    }
