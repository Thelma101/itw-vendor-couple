...
import { addRecentlyViewed } from '@/couple/components/home/RecentlyViewed';
...
const VendorProfile: React.FC = () => {
  ...
  const vendorData = useMemo(() => {
    ...
    return {
      ...
      id: matched.id,
      name: matched.name,
      type: matched.category,
      location: matched.location,
      rating: matched.rating,
      reviewCount: matched.reviewCount,
      startingPrice: priceNum,
      profileImage: matched.image,
      bio: `${matched.name} is a trusted ${matched.category.toLowerCase()} based in ${matched.location}. Browse their portfolio, packages, and availability to plan your day.`,
    };
  }, [routeId]);

  const packages = useMemo(() => activePackages(), []);
  const addOns = useMemo(() => activeAddOns(), []);
  ...
  return (
    <Box sx={{ backgroundColor: '#FFF6F9', minHeight: '100vh' }}>
      ...
      {/* Back Button */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: { xs: 2, md: 4 }, py: 2 }}>
        <BackButton
      ...
      >
        ...
        <Box sx={{ width: { xs: '100%', md: 340 }, flexShrink: 0, maxWidth: '100%' }}>
          <Card sx={{ 
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Card sx={{ 
            ...
            {addOns.length > 0 ? (
              <Box sx={{ mt: 3 }}>
                <Typography sx={{ fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: 15, color: '#002528', mb: 1.5 }}>
                  Optional add-ons
                </Typography>
                <Typography sx={{ fontFamily: 'var(--font-ui)', fontSize: 12, color: '#666', mb: 2 }}>
                  Stack these on any package — they are extras, not stand-alone bookings.
                </Typography>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 1.5 }}>
                  {addOns.map((addon) => (
                    ...
                    >
                      <Box>
                        <Typography sx={{ fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: 14, color: '#0B2D31' }}>
                          {addon.name}
                        </Typography>
                        <Typography sx={{ fontFamily: 'var(--font-ui)', fontSize: 12, color: '#666', mt: 0.5 }}>
                          {addon.description}
                        </Typography>
                      </Box>
                      <Typography sx={{ fontFamily: 'var(--font-ui)', fontWeight: 800, fontSize: 14, color: '#B45309', whiteSpace: 'nowrap' }}>
          ...
          <Card sx={{ 
            ...
            <Box sx={{ 
              ...
              {portfolioWorks.map((work, index) => (
                <Box
                  key={index}
                  onClick={() => setPortfolioLightbox(index)}
                  sx={{
                    position: 'relative',
                    height: 250,
                    overflow: 'hidden',
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3 }}>
              <Button
                onClick={() => setPortfolioLightbox(0)}
                sx={{
                  backgroundColor: '#00838F',
                  color: 'white',
                  fontFamily: "'Open Sans', sans-serif",
      ...
      >
        ...
        {portfolioLightbox !== null ? (
          <>
            <IconButton
              onClick={() =>
                setPortfolioLightbox((i) =>
                  i === null ? 0 : (i - 1 + portfolioWorks.length) % portfolioWorks.length,
                )
              }
            ...
            <IconButton
              onClick={() =>
                setPortfolioLightbox((i) => (i === null ? 0 : (i + 1) % portfolioWorks.length))
              }
              sx={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', color: '#fff', bgcolor: 'rgba(255,255,255,0.12)' }}
            >
              <ChevronRightIcon fontSize="large" />
            </IconButton>
            <Box sx={{ height: '100%', display: 'grid', placeItems: 'center', p: { xs: 2, md: 6 } }}>
              <Box
                component="img"
                src={portfolioWorks[portfolioLightbox].image}
                alt={portfolioWorks[portfolioLightbox].title || 'Portfolio'}
                sx={{ maxWidth: '100%', maxHeight: '90vh', objectFit: 'contain' }}
              />
            </Box>
            <Typography sx={{ position: 'absolute', bottom: 24, left: 0, right: 0, textAlign: 'center', color: '#fff', fontWeight: 700 }}>
              {portfolioLightbox + 1} / {portfolioWorks.length} · {vendorData.name}
            </Typography>
          </>
        ) : null}
      </Dialog>
